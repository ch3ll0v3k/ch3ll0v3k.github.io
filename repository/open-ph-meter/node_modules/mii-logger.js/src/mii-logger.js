...
console.shell = {
  ...
  stream: async ( cmd, args=[], onData )=>{
    return new Promise( async(resolve, reject)=>{   
      try{

        const proc = _chi.spawn( cmd, args );

        onData = (typeof onData === 'function' ? onData : ()=>{});

        proc.stdout.on('data', function (data) {
          try{
            // console.log('stdout: ' + data.toString());
            // onData({type: 'data', data: data.toString()}); 
            onData({type: 'data', data: data.toString().replace(/\\n/g, '\\n') }); 
          }catch(e){
        ...
        proc.stderr.on('data', function (data) {
          try{
            ...
            onData({type: 'error', data: data.toString()}); 
          }catch(e){
            console.warn(`warn: ${e.message}`);
          }
        });
        ...
        proc.on('exit', function (code) {
          try{
            ...
            onData({type: 'exited', data: code.toString()}); 
            resolve({code: code.toString()});
          }catch(e){
            console.warn(`warn: ${e.message}`);
          }
        });
